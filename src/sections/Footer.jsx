          ...
          <div className="flex items-center gap-5 mt-8 ">
            {socialMedia.map((icon) => (
              <div className="flex justify-center items-center w-12 h-12 bg-white rounded-full ">
                <img width={24} height={24} src={icon.src} alt={icon.src} />
              </div>
            ))}
          </div>
        </div>
            ...
            <div key={section}>
              ...
              <ul>
                {section.links.map((link) => (
                  <li
                    className="mt-3 text-white-400 font-montserrat hover:text-slate-gray text-base leading-normal cursor-pointer "
                    key={link.name}
                  >
                    <a href="">{link.name}</a>
                  </li>
